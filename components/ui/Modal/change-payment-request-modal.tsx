...
import { submitChangeRequest } from "lib/redux/paymentSlice"
...
const PATH_OPTIONS = [
  ...
  { value: "paymentTypeId", label: "Payment Type" },
  { value: "narrative", label: "Narrative" },
  { value: "externalReference", label: "External Reference" },
  { value: "paidAtUtc", label: "Paid At (UTC)" },
  { value: "collectorType", label: "Collector Type" },
]
      ...
      >
        ...
        <div className="max-h-[70vh] p-6">
          <div className="mb-6 text-center">
            <h3 className="mb-2 text-lg font-semibold text-gray-900">Request Changes for Payment</h3>
            <p className="text-sm text-gray-600">
              Payment Reference: <span className="font-medium">{paymentReference}</span>
            </p>
            <p className="mt-2 text-sm text-gray-600">
              Submit a change request for payment details. Changes will require approval.
            </p>
          </div>
    ...
    <div className="flex-1">
      {change.path === "channel" ? (
        <FormSelectModule
          ...
          options={[{ value: "", label: "Select channel" }, ...CHANNEL_OPTIONS]}
          required
          disabled={isLoading}
          error={errors[`change-${index}-value`]}
        />
      ) : change.path === "collectorType" ? (
        <FormSelectModule
          label="New Value"
          name={`change-${index}-value`}
          value={change.value}
          ...
          options={[{ value: "", label: "Select collector type" }, ...COLLECTOR_TYPE_OPTIONS]}
          required
          disabled={isLoading}
          error={errors[`change-${index}-value`]}
        />
