...
import { createTariffOverride } from "lib/redux/customerSlice"
import type { Customer, TariffOverrideRequest } from "lib/redux/customerSlice"

interface TariffOverrideModalProps {
  isOpen: boolean
  onRequestClose: () => void
  customerId: number
  customerName: string
...
type FormErrors = {
  [K in keyof TariffOverrideRequest]?: string
}

const TariffOverrideModal: React.FC<TariffOverrideModalProps> = ({
  isOpen,
  onRequestClose,
  customerId,
  customerName,
  ...
  const { tariffOverrideLoading, tariffOverrideSuccess } = useAppSelector((state) => state.customers)

  const [formData, setFormData] = useState<TariffOverrideRequest>({
    tariffRateOverride: 0,
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.tariffRateOverride || formData.tariffRateOverride <= 0) {
      newErrors.tariffRateOverride = "Tariff rate must be greater than 0"
    }
  ...
  const handleInputChange = (field: keyof TariffOverrideRequest, value: string | number) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.meta.requestStatus === "fulfilled") {
        notify("success", "Tariff override created successfully!", {
          duration: 3000,
        })
        onRequestClose()
        // Reset form
      ...
      console.error("Error creating tariff override:", error)
    }
  }

  const handleClose = () => {
    if (!tariffOverrideLoading) {
      onRequestClose()
      setErrors({})
      setFormData({
        tariffRateOverride: 0,
    ...
    >
      ...
      >
        <div className="flex w-full items-center justify-between bg-[#F9F9F9] p-4 sm:px-6 sm:py-5">
          <h2 className="text-base font-bold text-gray-900 sm:text-lg md:text-xl">Create Tariff Override</h2>
          <button
            onClick={handleClose}
            ...
            disabled={tariffOverrideLoading}
          >
        ...
        <div className="max-h-[80vh]">
          <div className="flex flex-col gap-4 px-4 pb-5 pt-5 sm:px-6 sm:pb-6 sm:pt-6">
            <div className="mb-2 w-full text-center">
              ...
              {currentCustomer?.tariff && (
                <p className="mt-1 text-xs text-gray-500 sm:text-sm">
                  Current Tariff: {currentCustomer.tariff.tariffRate} {currentCustomer.tariff.currency}
                </p>
              )}
            </div>

            <p className="text-xs text-gray-500 sm:text-sm">
              Create a temporary tariff override for this customer. This will override the standard tariff rate for the
              specified period.
            </p>

            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <FormInputModule
                label="Tariff Rate Override"
                name="tariffRateOverride"
                type="number"
                step="0.0001"
                min="0.0001"
                value={formData.tariffRateOverride}
                onChange={(e) => handleInputChange("tariffRateOverride", parseFloat(e.target.value) || 0)}
                placeholder="0.0000"
                required
                disabled={tariffOverrideLoading}
                error={errors.tariffRateOverride}
              ...
              <FormTextAreaModule
                ...
                placeholder="Please provide a reason for this tariff override"
                required
                disabled={tariffOverrideLoading}
                error={errors.reason}
                className="sm:col-span-2"
        ...
        >
          ...
          <ButtonModule
            ...
            disabled={tariffOverrideLoading}
          >
            {tariffOverrideLoading ? "Creating..." : "Create Override"}
          </ButtonModule>
        </form>
      </motion.div>
    </motion.div>
...
export default TariffOverrideModal
