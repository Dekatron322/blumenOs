// components/CustomerChangeRequestModal.tsx
"use client"
...
import { submitChangeRequest } from "lib/redux/customerSlice"
...
interface CustomerChangeRequestModalProps {
  isOpen: boolean
  onRequestClose: () => void
  customerId: number
  customerName: string
...
const PATH_OPTIONS = [
  ...
  { value: "addressTwo", label: "Address Line 2" },
  { value: "city", label: "City" },
  { value: "state", label: "State" },
  { value: "distributionSubstationId", label: "Distribution Substation" },
  { value: "serviceCenterId", label: "Service Center" },
  { value: "latitude", label: "Latitude" },
  { value: "longitude", label: "Longitude" },
  { value: "tariff", label: "Tariff" },
  { value: "meterNumber", label: "Meter Number" },
  { value: "isPPM", label: "PPM Status" },
  { value: "isMD", label: "MD Status" },
  { value: "comment", label: "Comment" },
  { value: "band", label: "Band" },
  { value: "storedAverage", label: "Stored Average" },
  { value: "totalMonthlyVend", label: "Total Monthly Vend" },
  { value: "totalMonthlyDebt", label: "Total Monthly Debt" },
  { value: "customerOutstandingDebtBalance", label: "Outstanding Debt Balance" },
  { value: "salesRepUserId", label: "Sales Representative" },
  { value: "technicalEngineerUserId", label: "Technical Engineer" },
]

export const CustomerChangeRequestModal: React.FC<CustomerChangeRequestModalProps> = ({
  isOpen,
  onRequestClose,
  customerId,
  customerName,
  ...
  const handleChangeUpdate = (index: number, field: keyof ChangeItem, value: string) => {
    const newChanges = changes.map((change, i) => (i === index ? { ...change, [field]: value } : change))
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-[700px] max-w-4xl rounded-lg bg-white shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between bg-[#F9F9F9] p-6">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Request Customer Changes</h2>
        ...
        <div className="max-h-[70vh]  p-6">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-600">
              Request changes to customer details. All changes will require approval before being applied.
            </p>
          </div>
...
export default CustomerChangeRequestModal
