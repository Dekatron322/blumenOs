...
interface AgentChangePasswordModalProps {
  isOpen: boolean
  onRequestClose: () => void
  onConfirm?: () => void
  userId: number
...
const AgentChangePasswordModal: React.FC<AgentChangePasswordModalProps> = ({
  isOpen,
  onRequestClose,
  onConfirm,
  userId,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Use the reset password action with the user ID
      ...
      if (resetEmployeePassword.fulfilled.match(result)) {
        notify("success", `Password for ${agentName} has been reset successfully`)
        onSuccess?.()
        onRequestClose()
      } else {
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          <div className="flex flex-col px-4 pb-4 pt-4 sm:px-6 sm:pb-6 sm:pt-6">
            ...
            <p className="mb-4 text-center text-sm text-gray-600 sm:mb-6 sm:text-base">
              Set a new password for <span className="font-semibold">{agentName}</span>
            </p>
...
export default AgentChangePasswordModal
