...
interface MakeRefundSuccessModalProps {
  isOpen: boolean
  onRequestClose: () => void
  refundData: any
  onSuccess?: () => void
...
const MakeRefundSuccessModal: React.FC<MakeRefundSuccessModalProps> = ({
  isOpen,
  onRequestClose,
  refundData,
  onSuccess,
    ...
    const thermalReceiptContent = `
      ...
      <body>
        ...
        <div class="row">
          ...
          <span class="row-value">${refundData.originalReference}</span>
        </div>
        <div class="row">
          <span class="row-label">Refund Ref:</span>
          <span class="row-value">${refundData.refundReference}</span>
        </div>
        <div class="row">
        ...
        <div class="row">
          ...
          <span class="row-value">${refundData.refundCount} / ${refundData.refundLimit}</span>
        </div>
  ...
  const handleDownloadPDF = async () => {
    if (!receiptRef.current || !refundData) return
    ...
    try {
      ...
      const fileName = `Refund-Receipt-${refundData.refundReference}.pdf`
      pdf.save(fileName)
    } catch (error) {
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-4 py-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Refund Successful</h2>
              <p className="text-sm text-gray-500">Refund has been processed successfully</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div ref={receiptRef} className="space-y-4 bg-[#EFEFEF] px-4 pb-4 sm:space-y-6 sm:px-6">
          ...
          <div className="rounded-lg bg-white p-4">
            <div className="mb-4 grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-semibold text-gray-900">{refundData.originalReference}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Refund Reference</p>
                <p className="font-semibold text-gray-900">{refundData.refundReference}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Refund Count</p>
                <p className="font-semibold text-gray-900">
                  {refundData.refundCount} of {refundData.refundLimit}
                </p>
              </div>
              <div>
          ...
          <div className="rounded-lg bg-white p-4">
            ...
            <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{refundData.receipt.customerName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Account:</span>
                <span className="font-medium">{refundData.receipt.customerAccountNumber}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Meter:</span>
                <span className="font-medium">{refundData.receipt.customerMeterNumber}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Phone:</span>
                <span className="font-medium">{refundData.receipt.customerPhoneNumber || "N/A"}</span>
              </div>
              {refundData.receipt.customerAddress && (
                <div className="col-span-1 sm:col-span-2">
                  <span className="text-gray-500">Address:</span>
                  <span className="ml-2 font-medium">{refundData.receipt.customerAddress}</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-2">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-gray-900">
                  {formatCurrency(refundData.receipt.totalAmountPaid, refundData.receipt.currency)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Units:</span>
                <span className="font-medium">{refundData.receipt.units} kWh</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Electricity:</span>
                <span className="font-medium">
                  {formatCurrency(refundData.receipt.electricityAmount, refundData.receipt.currency)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">VAT:</span>
                <span className="font-medium">
                  {formatCurrency(refundData.receipt.vatAmount, refundData.receipt.currency)}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="rounded-lg border-2 border-emerald-200 bg-emerald-50 p-4">
              ...
              <div className="rounded-lg bg-white p-4 text-center">
                <p className="mb-2 break-all font-mono text-2xl font-bold tracking-wider text-gray-900">
                  {refundData.receipt.token.token}
                </p>
                <div className="mt-3 flex flex-wrap justify-center gap-4 text-sm text-gray-600">
                  <span>
                    <strong>Amount:</strong> {refundData.receipt.token.vendedAmount} {refundData.receipt.token.unit}
                  </span>
                  <span>
                    <strong>Meter:</strong> {refundData.receipt.token.drn}
                  </span>
                </div>
                {refundData.receipt.token.description && (
                  <p className="mt-2 text-xs text-gray-500">{refundData.receipt.token.description}</p>
                )}
              </div>
            </div>
          )}
          ...
          {refundData.receipt.receipt?.tokens && refundData.receipt.receipt.tokens.length > 0 && (
            <div className="rounded-lg bg-white p-4">
              <h4 className="mb-3 font-semibold text-gray-900">All Tokens</h4>
              <div className="space-y-3">
                {refundData.receipt.receipt.tokens.map((token: Token, index: number) => (
...
export default MakeRefundSuccessModal
