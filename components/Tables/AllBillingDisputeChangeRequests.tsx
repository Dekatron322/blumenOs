...
import { ChangeRequestListItem, viewChangeRequests } from "lib/redux/billingDisputeSlice"
...
const ChangeRequestCardSkeleton = () => (
  <motion.div
    className="mt-3 rounded-lg border bg-[#f9f9f9] p-3 shadow-sm md:p-4"
    initial={{ opacity: 0.6 }}
    animate={{
      opacity: [0.6, 1, 0.6],
      transition: {
      ...
      <div className="flex items-start gap-3 md:items-center md:gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
            <div className="h-5 w-32 rounded bg-gray-200 md:h-6 md:w-40"></div>
            <div className="flex flex-wrap gap-1 md:gap-2">
              <div className="h-6 w-12 rounded-full bg-gray-200 md:w-16"></div>
              <div className="h-6 w-16 rounded-full bg-gray-200 md:w-20"></div>
              <div className="h-6 w-16 rounded-full bg-gray-200 md:w-24"></div>
            </div>
          </div>
...
const AllBillingDisputeChangeRequests = () => {
  const dispatch = useDispatch<AppDispatch>()

  const {
    viewChangeRequests: changeRequests,
    loadingViewChangeRequests,
    viewChangeRequestsError,
    viewChangeRequestsTotalPages,
    viewChangeRequestsTotalCount,
    viewChangeRequestsCurrentPage,
    viewChangeRequestsPageSize,
  } = useSelector((state: RootState) => state.billingDispute)
  ...
  useEffect(() => {
    dispatch(
      viewChangeRequests({
        ...
        PageSize: viewChangeRequestsPageSize || 12,
  ...
  }, [dispatch, currentPage, viewChangeRequestsPageSize, selectedStatus, selectedSource, searchText])
  ...
  const totalPages = viewChangeRequestsTotalPages || 1
  const totalRecords = viewChangeRequestsTotalCount || 0
  const pageSize = viewChangeRequestsPageSize || 12
  ...
  const ChangeRequestListItemComponent = ({ changeRequest }: { changeRequest: ChangeRequestListItem }) => {
    const statusConfig = getStatusConfig(changeRequest.status)
    const sourceConfig = getSourceConfig(changeRequest.source || 1)
  ...
  return (
    <>
        ...
        <div className="w-full rounded-md border bg-white p-3 md:p-5">
            ...
            <div className="mb-3 flex w-full items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    /* TODO: Implement CSV export for billing dispute change requests */
          ...
          {viewChangeRequestsError && (
            <div className="mb-4 rounded-md bg-red-50 p-3 text-sm text-red-700 md:p-4 md:text-base">
              <p>Error loading change requests: {viewChangeRequestsError}</p>
            </div>
          )}
          ...
          <div className="w-full">
            {changeRequests.length === 0 ? (
              ...
              <div className="divide-y">
                {changeRequests.map((changeRequest: ChangeRequestListItem) => (
                  <ChangeRequestListItemComponent key={changeRequest.publicId} changeRequest={changeRequest} />
                ))}
              </div>
            )}
          </div>
          ...
          {changeRequests.length > 0 && (
            <div className="mt-4 flex w-full flex-row items-center justify-between gap-3 md:flex-row">
              <div className="flex items-center gap-1 max-sm:hidden">
                ...
                <select value={pageSize} onChange={handleRowsChange} className="bg-[#F2F2F2] p-1 text-sm md:text-base">
      ...
      {/* View Billing Dispute Change Request Modal */}
      <ViewBillingDisputeChangeRequestModal
        isOpen={isModalOpen}
...
export default AllBillingDisputeChangeRequests
