...
interface AllPendingCollectionsTableProps {
  agentId?: number
  customerId?: number
...
const AllPendingCollectionsTable: React.FC<AllPendingCollectionsTableProps> = ({
  agentId,
  customerId,
  ...
  useEffect(() => {
    const fetchParams: PaymentsRequestParams = {
      ...
      // Always filter for pending status for this component
      status: PaymentStatus.Pending,
      // Applied filters from parent component (but don't override status)
        ...
        {pendingPayments.length > 0 && (
          ...
          >
            <div className="rounded-lg border border-[#004B23]/20 bg-[#004B23]/5 p-4 shadow-sm">
              <div className="text-xs font-medium uppercase tracking-wide text-[#004B23]/80">Total Pending Amount</div>
            ...
            <div className="rounded-lg border border-amber-100 bg-amber-50 p-4 shadow-sm">
              <div className="text-xs font-medium uppercase tracking-wide text-amber-700">Pending Count</div>
              <div className="mt-1 text-xl font-semibold text-amber-900">{pendingPayments.length}</div>
            </div>
            <div className="rounded-lg border border-blue-100 bg-blue-50 p-4 shadow-sm">
              <div className="text-xs font-medium uppercase tracking-wide text-blue-700">Avg Amount</div>
              <div className="mt-1 text-xl font-semibold text-blue-900">
                {formatCurrency(
                  pendingPayments.reduce((sum, payment) => sum + payment.amount, 0) / pendingPayments.length
                )}
              </div>
            </div>
            <div className="rounded-lg border border-purple-100 bg-purple-50 p-4 shadow-sm">
              <div className="text-xs font-medium uppercase tracking-wide text-purple-700">Collections Today</div>
              <div className="mt-1 text-xl font-semibold text-purple-900">
                {
                  pendingPayments.filter((p) => {
                    const paymentDate = new Date(p.paidAtUtc)
...
export default AllPendingCollectionsTable
