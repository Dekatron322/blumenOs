...
const DistributionStationTab: React.FC = () => {
  ...
  // Fetch companies, area offices, injection substations, and feeders for filter options
  useEffect(() => {
    dispatch(
      fetchCompanies({
        pageNumber: 1,
  ...
  // Extract unique station codes from distribution substations for filter options (client-side only)
  const uniqueStationCodes = React.useMemo(() => {
    if (!distributionSubstations || distributionSubstations.length === 0) return []
    const stationMap = new Map<number, { id: number; name: string }>()
    distributionSubstations.forEach((station) => {
      if (station.id && station.dssCode) {
        if (!stationMap.has(station.id)) {
          stationMap.set(station.id, {
            id: station.id,
            name: station.dssCode,
          })
        }
      }
    })
  ...
  // Status options for filter (based on API status values)
  const statusOptions = [
  ...
  // Client-side filtering for station code (status is filtered server-side via API)
  const filteredSubstations = React.useMemo(() => {
    let filtered = distributionSubstations

    // Filter by station code ID (client-side only)
    if (appliedFilters.stationCodeId) {
      filtered = filtered.filter((station) => station.id.toString() === appliedFilters.stationCodeId)
    }

    // Status is filtered server-side via API, but we can also filter client-side as backup
    if (appliedFilters.status) {
      filtered = filtered.filter((station) => station.status === appliedFilters.status)
    }

    return filtered
  }, [distributionSubstations, appliedFilters.stationCodeId, appliedFilters.status])

  // Fetch distribution substations on component mount and when applied filters/pagination change
  useEffect(() => {
  ...
  const sortOptions: SortOption[] = [
    { label: "DSS Code A-Z", value: "dssCode", order: "asc" },
    { label: "DSS Code Z-A", value: "dssCode", order: "desc" },
    { label: "NERC Code A-Z", value: "nercCode", order: "asc" },
    { label: "NERC Code Z-A", value: "nercCode", order: "desc" },
    { label: "Capacity Low-High", value: "transformerCapacityInKva", order: "asc" },
    { label: "Capacity High-Low", value: "transformerCapacityInKva", order: "desc" },
    { label: "Feeder A-Z", value: "feeder", order: "asc" },
    { label: "Feeder Z-A", value: "feeder", order: "desc" },
  ]
        ...
        >
          ...
          >
            ...
            <div className="w-full">
              <SearchModule
                prominent
                prominentTitle="Search Distribution Stations"
                prominentDescription="Find distribution stations quickly by code, feeder, location, or status."
                value={searchInput}
                    ...
                    <thead>
                      <tr>
                        ...
                        <th className="text-500 whitespace-nowrap border-b p-4 text-sm">
                          <div className="flex items-center gap-2">DSS Code</div>
                        </th>
                        <th className="whitespace-nowrap border-b p-4 text-sm">
                          <div className="flex items-center gap-2">NERC Code</div>
                        </th>
                        <th className="whitespace-nowrap border-b p-4 text-sm">
                          <div className="flex items-center gap-2">Capacity (kVA)</div>
                        </th>
                        <th className="whitespace-nowrap border-b p-4 text-sm">
                          <div className="flex items-center gap-2">Feeder</div>
                        </th>
                        <th className="whitespace-nowrap border-b p-4 text-sm">
                      ...
                      <AnimatePresence>
                        {filteredSubstations.map((substation, index) => (
                          ...
                          >
                            ...
                            <td className="whitespace-nowrap border-b px-4 py-2 text-sm">
                              {(() => {
                                const stationStatus = substation.status || "operational"
                                const statusConfig = getStatusStyle(stationStatus)
                                const statusLabel = getStatusDisplayText(stationStatus)
                                const statusColorMap: { [key: string]: string } = {
                                  operational: "#589E67",
                                  maintenance: "#D97706",
                                  outage: "#AF4B4B",
                                  planned_shutdown: "#3B82F6",
                                }
                                return (
                                  <motion.div
                                    style={statusConfig}
