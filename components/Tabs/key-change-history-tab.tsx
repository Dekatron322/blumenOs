...
import { clearAddKeyChange, clearKeyChangeHistory, fetchKeyChangeHistory } from "lib/redux/metersSlice"
...
import KeyChangeDetailsModal from "components/ui/Modal/key-change-details-modal"
...
const KeyChangeHistoryTab: React.FC<KeyChangeHistoryTabProps> = ({ meterId }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      !keyChangeHistoryLoading &&
        !keyChangeHistoryError &&
  ...
  const KeyChangeListItem = ({ event }: { event: ClearTamperHistoryEntry }) => {
    const statusConfig = getStatusConfig(event.isSuccessful)
  ...
  if (keyChangeHistoryError) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">{keyChangeHistoryError}</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      {showFilters && (
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-700">Change Type</label>
            ...
            <div>
              ...
              <select className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                ...
                <option>Regular Rotation</option>
      ...
      {paginatedData.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12">
          <div className="flex size-12 items-center justify-center rounded-full bg-gray-100">
            <Key className="size-6 text-gray-400" />
          </div>
          <h3 className="mt-3 text-lg font-medium text-gray-900">No key change events found</h3>
          <p className="mt-1 text-sm text-gray-500">No key change events recorded for this meter</p>
        </div>
      ) : (
        <>
                    ...
                    <tr key={event.id} className="transition-colors hover:bg-gray-50">
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-xs font-medium text-gray-900">Key Change #{event.id}</p>
                      ...
                      <td className="px-3 py-2">
                        {event.customer ? (
                          <div>
                            <p className="text-xs font-medium text-gray-900">{event.customer.fullName}</p>
                            <p className="text-xs text-gray-500">{event.customer.accountNumber}</p>
                          </div>
                        ) : (
                          <p className="text-xs text-gray-500">No customer info</p>
                        )}
                      </td>
                      <td className="px-3 py-2">
                        <div className="text-xs">
                          <p className="font-medium text-gray-900">
                            {event.meterNumber || responseInfo.drn || event.meterId}
                          </p>
                        </div>
                      </td>
                      <td className="px-3 py-2">
      ...
      {/* Key Change Details Modal */}
      <KeyChangeDetailsModal
      ...
      {/* Success Modal */}
      <KeyChangeSuccessModal
        isOpen={showSuccessModal}
        onRequestClose={handleCloseSuccessModal}
        tokenData={addKeyChangeData}
      />
    </div>
  )
}

export default KeyChangeHistoryTab
