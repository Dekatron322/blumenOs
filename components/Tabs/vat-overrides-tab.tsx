...
import { clearAllVatOverridesState, fetchAllVatOverrides } from "lib/redux/customerSlice"
import { fetchEmployeeById } from "lib/redux/employeeSlice"
import type { VatOverrideData } from "lib/redux/customerSlice"

interface VatOverridesTabProps {
  customerId: number
  customerName: string
}

const VatOverridesTab: React.FC<VatOverridesTabProps> = ({ customerId, customerName }) => {
  const dispatch = useAppDispatch()
  const { allVatOverrides, allVatOverridesLoading, allVatOverridesError, allVatOverridesSuccess } = useAppSelector(
  ...
  useEffect(() => {
    // Fetch VAT overrides when component mounts
    dispatch(fetchAllVatOverrides(customerId))

    // Cleanup on unmount
    return () => {
      dispatch(clearAllVatOverridesState())
    }
  }, [dispatch, customerId])
  ...
  useEffect(() => {
    if (allVatOverrides.length > 0) {
      allVatOverrides.forEach((override) => {
  ...
  const getStatusBadge = (isActive: boolean, effectiveFrom: string, effectiveTo: string, isVatWaived: boolean) => {
    const now = new Date()
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Error Loading VAT Overrides</h3>
            <p className="mt-1 text-sm text-gray-600">{allVatOverridesError}</p>
          </div>
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">No VAT Overrides</h3>
            <p className="mt-1 text-sm text-gray-600">{customerName} doesn&apos;t have any VAT overrides yet.</p>
          </div>
          <ButtonModule variant="primary" onClick={handleRefresh} className="mt-2">
            <RefreshCw className="mr-2 size-4" />
            Refresh
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Manage VAT overrides for {customerName}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-xs text-gray-500">Total Overrides</p>
            <p className="text-lg font-semibold text-gray-900">{allVatOverrides.length}</p>
          </div>
          <ButtonModule variant="secondary" onClick={handleRefresh} disabled={allVatOverridesLoading}>
            <RefreshCw className={`mr-2 size-4 ${allVatOverridesLoading ? "animate-spin" : ""}`} />
            Refresh
          </ButtonModule>
        </div>
      </div>
      ...
      <div className="grid gap-4">
        <AnimatePresence>
          {allVatOverrides.map((override: VatOverrideData, index: number) => {
            ...
            const isWaived = override.isVatWaived
                    ...
                    <div className="flex items-start gap-4">
                      <div
                        className={`rounded-xl p-3 ${
                          isExpired ? "bg-gray-100" : isWaived ? "bg-red-100" : "bg-purple-100"
                        }`}
                      >
                        {isWaived ? (
                          <Ban className={`size-6 ${isExpired ? "text-gray-600" : "text-red-600"}`} />
                        ) : (
                          <Percent className={`size-6 ${isExpired ? "text-gray-600" : "text-purple-600"}`} />
                        )}
                      </div>
                      <div className="flex-1">
                        <div className="mb-2 flex items-center gap-3">
                          {getStatusBadge(
                            override.isActive,
                            override.effectiveFromUtc,
                            override.effectiveToUtc,
                            override.isVatWaived
                          )}
                          {isWaived ? (
                            <h3 className="text-xl font-bold text-red-600">VAT Waived</h3>
                          ) : (
                            <h3 className="text-xl font-bold text-gray-900">
                              {override.vatRateOverride}%
                              <span className="ml-2 text-sm font-normal text-gray-500">VAT rate</span>
                            </h3>
                          )}
                        </div>
...
export default VatOverridesTab
