...
const VendorChangeRequestListItem = ({
  changeRequest,
  onViewDetails,
}: {
  changeRequest: ChangeRequestListItem
...
interface VendorChangeRequestsTabProps {
  vendorId: number
}

const VendorChangeRequestsTab: React.FC<VendorChangeRequestsTabProps> = ({ vendorId }) => {
  const dispatch = useAppDispatch()
  const {
    changeRequestsByVendor,
    changeRequestsByVendorLoading,
    changeRequestsByVendorError,
  ...
  useEffect(() => {
    const params: ChangeRequestsRequestParams = {
      ...
      pageSize: changeRequestsByVendorPagination.pageSize,
    ...
    dispatch(fetchChangeRequestsByVendorId({ id: vendorId, params }))
  }, [
    dispatch,
    vendorId,
    currentPage,
    changeRequestsByVendorPagination.pageSize,
    selectedStatus,
    selectedSource,
    searchText,
  ])
  ...
  useEffect(() => {
    return () => {
      dispatch(clearChangeRequestsByVendor())
    }
  }, [dispatch])
  ...
  const totalPages = changeRequestsByVendorPagination.totalPages || 1
  const totalRecords = changeRequestsByVendorPagination.totalCount || 0
  ...
  if (changeRequestsByVendorLoading) {
    return (
      <motion.div
      ...
      >
        <h3 className="mb-6 flex items-center gap-2 text-lg font-semibold text-gray-900">Vendor Change Requests</h3>
    ...
    <>
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-lg font-semibold text-gray-900">Vendor Change Requests</h3>
          <button
            ...
            disabled={!changeRequestsByVendor || changeRequestsByVendor.length === 0}
          >
        ...
        {changeRequestsByVendorError ? (
          <div className="py-8 text-center">
            ...
            <p className="text-gray-500">Error loading change requests: {changeRequestsByVendorError}</p>
          </div>
        ) : changeRequestsByVendor.length === 0 ? (
          <div className="py-8 text-center">
            <p className="text-gray-500">No change requests found for this vendor</p>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {changeRequestsByVendor.map((changeRequest: ChangeRequestListItem) => (
              <VendorChangeRequestCard
        ...
        {changeRequestsByVendor.length > 0 && (
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-center gap-1">
              <p>Show rows</p>
              <select
                value={changeRequestsByVendorPagination.pageSize}
...
export default VendorChangeRequestsTab
