...
interface SetControlHistoryTabProps {
  meterId: number
}

const SetControlHistoryTab: React.FC<SetControlHistoryTabProps> = ({ meterId }) => {
  ...
  const [showSetControlModal, setShowSetControlModal] = useState(false)
  ...
  const handleSetControl = async (controlData: SetControlRequest) => {
    await dispatch(setControl({ id: meterId, controlData }))
    if (!setControlError) {
  ...
  const getStatusConfig = (isSuccessful: boolean) => {
    return isSuccessful
      ? { color: "text-emerald-600", bg: "bg-emerald-50", label: "Successful" }
      : { color: "text-red-600", bg: "bg-red-50", label: "Failed" }
  }
  ...
  const ControlListItem = ({ event }: { event: ClearTamperHistoryEntry }) => {
    const statusConfig = getStatusConfig(event.isSuccessful)
      ...
      <div className="border-b bg-white p-4 transition-all hover:bg-gray-50">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex size-10 items-center justify-center rounded-full bg-purple-100 max-sm:hidden">
              <Settings />
            </div>
  ...
  if (setControlHistoryError) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">{setControlHistoryError}</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {showFilters && (
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-700">Control Type</label>
              <select className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                <option>All Types</option>
                <option>Remote Disconnect</option>
                <option>Remote Reconnect</option>
                <option>Tariff Change</option>
                <option>Parameter Update</option>
              </select>
            </div>
            <div>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-700">Command</label>
              <select className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                <option>All Commands</option>
                <option>Disconnect</option>
                <option>Reconnect</option>
                <option>Set Tariff</option>
                <option>Update Parameter</option>
              </select>
            </div>
            <div>
      ...
      {paginatedData.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="mt-3 text-lg font-medium text-gray-900">No control events found</h3>
          <p className="mt-1 text-sm text-gray-500">No control operations recorded for this meter</p>
        </div>
      ) : (
        <>
          {viewMode === "list" ? (
            <div className="divide-y">
              {paginatedData.map((event) => (
                <ControlListItem key={event.id} event={event} />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {paginatedData.map((event) => (
                <ControlCard key={event.id} event={event} />
              ))}
            </div>
          )}
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="mx-4 max-w-lg rounded-lg bg-white p-6">
            <h3 className="mb-4 text-lg font-semibold">Set Control Event Details</h3>
      ...
      {/* Set Control Modal */}
      <SetControlModal
        isOpen={showSetControlModal}
        onRequestClose={() => setShowSetControlModal(false)}
...
export default SetControlHistoryTab
