...
import { clearTamper, clearTamperData as clearTamperDataAction, fetchMeters } from "lib/redux/metersSlice"
import {
  createFileIntent,
  FileIntentResponse,
  finalizeFile,
  processClearTamperBulkUpload,
...
const ClearTamper = () => {
  ...
  const { meters, loading, error, clearTamperLoading, clearTamperError, clearTamperData } = useAppSelector(
  ...
  const {
    ...
    fileIntentError,
    finalizeFileLoading,
    clearTamperBulkUploadLoading,
    clearTamperBulkUploadError,
    clearTamperBulkUploadSuccess,
    clearTamperBulkUploadResponse,
    csvJobs,
    csvJobsLoading,
    csvJobsError,
    csvJobsSuccess,
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(resetClearTamperBulkUploadState())
    }
  }, [dispatch])
  ...
  const handleDownloadCsv = async (job: any) => {
    try {
      const result = await dispatch(downloadClearTamper({ id: job.id }))
      if (downloadClearTamper.fulfilled.match(result)) {
        const response = result.payload

        // Check if the file is ready for download (has blob data)
        if (response.data instanceof Blob && response.fileName) {
          ...
          notify("success", `File "${response.fileName}" downloaded successfully`)
        } else if (response.message) {
          // File not ready or other API response - show the exact message
          const notificationType = response.isSuccess ? "warning" : "error"
          notify(notificationType, response.message)
        } else {
          // Fallback for unexpected response
          notify("error", "Unexpected response from server")
        }
      } else if (downloadClearTamper.rejected.match(result)) {
        // Handle actual errors (network issues, 500, etc.)
  ...
  // Filter jobs to only show clear tamper job types
  const clearTamperJobTypes = [26] // Clear Tamper job type values
  const filteredCsvJobs = csvJobs.filter((job) => clearTamperJobTypes.includes(job.jobType))
  ...
  const handleClearTamper = async () => {
    ...
    try {
      const result = await dispatch(clearTamper(selectedMeter.id))
      ...
      notify("error", "Error clearing tamper")
    }
  }
  ...
  // Bulk upload handler functions
  const handleFileSelect = (file: File) => {
    setSelectedFile(file)
    setUploadError(null)
    setUploadSuccess(false)
    // Reset file management state when new file is selected
    dispatch(resetClearTamperBulkUploadState())
  }
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (file && (file.type === "text/csv" || file.name.endsWith(".xlsx") || file.name.endsWith(".xls"))) {
      ...
      notify("error", "Please upload a CSV or Excel file")
    }
  }
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const fileIntentRequest = {
        ...
        purpose: "meters-clear-tamper",
        checksum: await calculateFileChecksum(selectedFile),
        bulkInsertType: "clear-tamper",
        columns: ["meterNumber"], // Required columns for clear tamper
      }
      ...
      // Step 4: Process clear tamper bulk upload
      const bulkUploadResult = await dispatch(processClearTamperBulkUpload({ fileId }))
      ...
      setUploadError(error.message || "Failed to process bulk clear tamper")
      notify("error", error.message || "Failed to process bulk clear tamper")
    } finally {
  ...
  const downloadSampleTemplate = () => {
    ...
    link.setAttribute("download", "clear-tamper-template.csv")
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h1 className="text-2xl font-bold text-gray-900">Clear Tamper</h1>
                      <p className="text-sm text-gray-600">Clear tamper status for single or multiple meters</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900">Single Clear</h3>
                        <p className="mt-1 text-sm text-gray-600">
                          Clear tamper status for individual meters using search
                        </p>
                        {activeTab === "single" && (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900">Bulk Clear</h3>
                        <p className="mt-1 text-sm text-gray-600">
                          Clear tamper status for multiple meters using file upload
                        </p>
                        {activeTab === "bulk" && (
                          <div className="mt-3 flex items-center gap-2">
                            <span className="inline-flex items-center rounded-full bg-[#004B23] px-2 py-1 text-xs font-medium text-white">
              ...
              <AnimatePresence mode="wait">
                {activeTab === "single" ? (
                  ...
                  >
                    ...
                    {clearTamperError && (
                      <div className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4">
                        <h3 className="text-sm font-medium text-red-800">Error clearing tamper</h3>
                        <p className="text-sm text-red-600">{clearTamperError}</p>
                      </div>
                    )}
                  ...
                  <motion.div
                    ...
                    transition={{ duration: 0.2 }}
                  >
                    {/* Bulk Clear Tamper Content */}
                    <div className="mb-6">
                      <div className="mb-4 flex items-center justify-between">
                        <div>
                          <h3 className="text-lg font-medium text-gray-900">Upload File</h3>
                          <p className="text-sm text-gray-600">
                            Upload a CSV file with meter numbers to clear tamper status
                          </p>
                        </div>
                        <ButtonModule
                    ...
                    {/* Clear Tamper Bulk Upload Error */}
                    {clearTamperBulkUploadError && (
                      <div className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4">
                        <h3 className="text-sm font-medium text-red-800">Bulk Upload Error</h3>
                        <p className="text-sm text-red-600">{clearTamperBulkUploadError}</p>
                      </div>
                    )}
                    ...
                    {uploadSuccess && (
                      <div className="mb-6 rounded-lg border border-green-200 bg-green-50 p-4">
                        ...
                        <p className="text-sm text-green-600">Bulk clear tamper processed successfully</p>
                        {clearTamperBulkUploadResponse && (
                          <div className="mt-2 text-sm text-green-600">
                            <p>Job ID: {clearTamperBulkUploadResponse.data.id}</p>
                            <p>
                              Status: {clearTamperBulkUploadResponse.data.status === 1 ? "Processing" : "Completed"}
                            </p>
                            <p>Total Rows: {clearTamperBulkUploadResponse.data.totalRows}</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {/* Upload Progress */}
                    {(isUploading || fileIntentLoading || finalizeFileLoading || clearTamperBulkUploadLoading) && (
                      <div className="mb-6">
                    ...
                    <div className="mt-8">
                      <div className="mb-6 rounded-lg border bg-white">
                        ...
                        <div className="border-b p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <h3 className="text-lg font-semibold">Clear Tamper Jobs</h3>
                              {csvJobsPagination && (
                                <p className="text-sm text-gray-600">
