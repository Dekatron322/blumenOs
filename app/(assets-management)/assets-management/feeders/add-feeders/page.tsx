...
import { clearCreateState, createFeeder, createSingleFeeder, fetchFeeders } from "lib/redux/feedersSlice"
...
const AddFeederPage = () => {
  ...
  const [csvData, setCsvData] = useState<CSVFeeder[]>([])
  ...
  // Fetch existing feeders, injection substations, and HT poles on component mount
  useEffect(() => {
    dispatch(
      fetchFeeders({
        pageNumber: 1,
  ...
  const handleSingleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    await submitSingleFeeder()
  }

  const submitSingleFeeder = async () => {
    if (!validateForm()) {
    ...
    try {
      const result = await dispatch(createSingleFeeder(formData)).unwrap()

      if (result.isSuccess) {
        notify("success", "Feeder created successfully", {
      ...
      if (!error?.includes("Network error")) {
        notify("error", "Failed to create feeder", {
      ...
      try {
        ...
        if (errors.length === 0) {
          notify("success", "CSV file parsed successfully", {
            description: `Found ${parsedData.length} valid feeder records`,
            duration: 4000,
          })
        } else {
  ...
  const validateCSVRow = (row: any, rowNumber: number): string[] => {
    ...
    if (!row.injectionsubstationid?.trim()) {
      errors.push(`Row ${rowNumber}: Injection Substation ID is required`)
    } else if (isNaN(parseInt(row.injectionsubstationid))) {
      errors.push(`Row ${rowNumber}: Injection Substation ID must be a valid number`)
    }

    if (!row.htpoleid?.trim()) {
      errors.push(`Row ${rowNumber}: HT Pole ID is required`)
    } else if (isNaN(parseInt(row.htpoleid))) {
      errors.push(`Row ${rowNumber}: HT Pole ID must be a valid number`)
    }

    if (!row.name?.trim()) {
      errors.push(`Row ${rowNumber}: Feeder name is required`)
    }
    ...
    if (!row.kaedcofeedercode?.trim()) {
      errors.push(`Row ${rowNumber}: KAEDCO feeder code is required`)
    }

    if (!row.feedervoltage?.trim()) {
      errors.push(`Row ${rowNumber}: Feeder voltage is required`)
    } else if (isNaN(parseFloat(row.feedervoltage))) {
      errors.push(`Row ${rowNumber}: Feeder voltage must be a valid number`)
    } else if (parseFloat(row.feedervoltage) < 0) {
      errors.push(`Row ${rowNumber}: Feeder voltage must be a positive number`)
    }
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      const result = await dispatch(createFeeder(csvData)).unwrap()

      if (result.isSuccess) {
        notify("success", "Feeders created successfully", {
          description: `${csvData.length} feeders have been added to the system`,
          duration: 6000,
        })
  ...
  const downloadTemplate = () => {
    const headers = ["injectionSubstationId", "htPoleId", "name", "nercCode", "kaedcoFeederCode", "feederVoltage"]

    const exampleData = [
      {
        injectionSubstationId: "1",
        htPoleId: "1",
        name: "Feeder 001",
        nercCode: "NERC_FDR_001",
        kaedcoFeederCode: "KAEDCO_FDR_001",
        feederVoltage: "11000",
      },
      {
        injectionSubstationId: "1",
        htPoleId: "2",
        name: "Feeder 002",
        nercCode: "NERC_FDR_002",
        kaedcoFeederCode: "KAEDCO_FDR_002",
        feederVoltage: "11000",
      },
      {
        injectionSubstationId: "2",
        htPoleId: "3",
        name: "Feeder 003",
        ...
        <div className="flex w-full flex-col">
          ...
          <div className="container mx-auto flex flex-col">
            ...
            <div className="flex w-full justify-between gap-6 px-16 max-md:flex-col max-md:px-0 max-sm:my-4 max-sm:px-3 md:my-8">
              <div>
                <h4 className="text-2xl font-semibold">Add New Feeder</h4>
                <p className="text-gray-600">Add a new feeder to the system</p>
              </div>
                ...
                {activeTab === "single" ? (
                  ...
                  >
                    ...
                    <div className="mb-6 border-b pb-4">
                      ...
                      <p className="text-sm text-gray-600">Fill in all required fields to add a new feeder</p>
                    </div>
                    ...
                    <form onSubmit={handleSingleSubmit} className="space-y-8">
                      ...
                      <div className="space-y-6 rounded-lg bg-[#f9f9f9] p-6">
                        <div className="border-b pb-4">
                          ...
                          <p className="text-sm text-gray-600">
                            Enter the feeder identification and technical information
                          </p>
                        </div>
                      ...
                      <div className="flex justify-end gap-4 border-t pt-6">
                        ...
                        >
                          {createLoading ? "Creating Feeder..." : "Create Feeder"}
                        </ButtonModule>
                      </div>
                    </form>
                  </motion.div>
                          ...
                          <div className="mt-4 flex justify-center gap-3">
                            ...
                            {csvErrors.length === 0 && csvData.length > 0 && (
                              <ButtonModule variant="primary" onClick={handleBulkSubmit} disabled={createLoading}>
                                {createLoading ? "Processing..." : `Create ${csvData.length} Feeders`}
                              </ButtonModule>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="max-h-48 overflow-y-auto">
                          <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                              {csvData.slice(0, 5).map((feeder, index) => (
                                <tr key={index}>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
                                    {feeder.injectionSubstationId}
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
                                    {feeder.htPoleId}
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">{feeder.name}</td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
                                    {feeder.nercCode}
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
