...
import {
  ...
  fetchAreaOfficeById,
  fetchChangeRequestsByAreaOfficeId,
} from "lib/redux/areaOfficeSlice"
import { AreaOfficeChangeRequestModal } from "components/ui/Modal/area-office-change-request-modal"
...
const AreaOfficeChangeRequestsSection = ({ areaOfficeId }: { areaOfficeId: number }) => {
  ...
  // Fetch change requests for this area office
  useEffect(() => {
    const params: ChangeRequestsRequestParams = {
      pageNumber: currentPage,
      pageSize: changeRequestsByAreaOfficePagination.pageSize,
    ...
    areaOfficeId,
    currentPage,
    changeRequestsByAreaOfficePagination.pageSize,
    selectedStatus,
    selectedSource,
    searchText,
  ])
  ...
  useEffect(() => {
    return () => {
      dispatch(clearChangeRequestsByAreaOffice())
    }
  }, [dispatch])
  ...
  const totalPages = changeRequestsByAreaOfficePagination.totalPages || 1
  const totalRecords = changeRequestsByAreaOfficePagination.totalCount || 0
  ...
  if (changeRequestsByAreaOfficeLoading) {
    return (
      <motion.div
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            ...
            onClick={() => {
              /* TODO: Implement CSV export for area office change requests */
            }}
            disabled={!changeRequestsByAreaOffice || changeRequestsByAreaOffice.length === 0}
          >
        ...
        {changeRequestsByAreaOfficeError ? (
          <div className="py-8 text-center">
            ...
            <p className="text-gray-500">Error loading change requests: {changeRequestsByAreaOfficeError}</p>
          </div>
        ) : changeRequestsByAreaOffice.length === 0 ? (
          <div className="py-8 text-center">
            <p className="text-gray-500">No change requests found for this area office</p>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {changeRequestsByAreaOffice.map((changeRequest: ChangeRequestListItemType) => (
              <ChangeRequestCard
        ...
        {changeRequestsByAreaOffice.length > 0 && (
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-center gap-1">
              <p>Show rows</p>
              <select
                value={changeRequestsByAreaOfficePagination.pageSize}
...
const AreaOfficeDetailsPage = () => {
  ...
  return (
    <section className="size-full">
            ...
            <div className="flex w-full px-16 py-8">
              <div className="flex w-full gap-6">
                ...
                <div className="flex w-full flex-col space-y-6 xl:col-span-2">
                  ...
                  <AreaOfficeChangeRequestsSection areaOfficeId={currentAreaOffice.id} />
                </div>
              </div>
            </div>
          </div>
      ...
      <AreaOfficeChangeRequestModal
        isOpen={activeModal === "changeRequest"}
        onRequestClose={closeAllModals}
        areaOfficeId={currentAreaOffice.id}
        areaOfficeName={currentAreaOffice.nameOfNewOAreaffice}
