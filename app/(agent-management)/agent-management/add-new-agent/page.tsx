...
const AddNewAgent = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.idNumber.trim()) {
      ...
      errors.idNumber = "ID number must be at least 11 characters"
    }
    ...
    if (!formData.initialFloat.trim()) {
      errors.initialFloat = "Initial float is required"
    } else if (parseFloat(formData.initialFloat.replace(/[₦,]/g, "")) <= 0) {
      errors.initialFloat = "Initial float must be greater than 0"
    }
  ...
  const handleSingleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    await submitSingleAgent()
  }

  const submitSingleAgent = async () => {
    if (!validateForm()) {
    ...
    try {
      ...
      console.log("Agent data ready for submission:", {
        ...formData,
        initialFloat: parseFloat(formData.initialFloat.replace(/[₦,]/g, "")),
        commissionRate: parseFloat(formData.commissionRate),
      })

      notify("success", "Agent created successfully", {
        description: `${formData.fullName} has been registered as an agent`,
        duration: 5000,
      })
      ...
      const errorMessage = error?.data?.message || "An unexpected error occurred while adding the agent"
      ...
      try {
        ...
        if (errors.length === 0) {
          notify("success", "CSV file parsed successfully", {
            description: `Found ${parsedData.length} valid agent records`,
            duration: 4000,
          })
        } else {
  ...
  const validateCSVRow = (row: any, rowNumber: number): string[] => {
    ...
    if (!row.idnumber?.trim()) {
      errors.push(`Row ${rowNumber}: ID number is required`)
    } else if (row.idnumber.length < 11) {
      errors.push(`Row ${rowNumber}: ID number must be at least 11 characters`)
    }

    if (!row.territory?.trim()) {
      errors.push(`Row ${rowNumber}: Territory is required`)
    }
    ...
    if (!row.initialfloat?.trim()) {
      errors.push(`Row ${rowNumber}: Initial float is required`)
    } else if (parseFloat(row.initialfloat.replace(/[₦,]/g, "")) <= 0) {
      errors.push(`Row ${rowNumber}: Initial float must be greater than 0`)
    }
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      console.log("Bulk agent data ready for upload:", csvData)

      notify("success", "Bulk upload ready", {
        description: `${csvData.length} agents validated and ready for upload. Backend integration pending.`,
        duration: 6000,
      })
        ...
        <div className="flex w-full flex-col">
          ...
          <div className="flex w-full  justify-between gap-6 px-16 max-md:flex-col max-md:px-0 max-sm:my-4 max-sm:px-3 md:my-8">
            <div>
              ...
              <p className="text-gray-600">Add a new field agent to the system</p>
            </div>
                ...
                {activeTab === "single" ? (
                  ...
                  >
                    ...
                    <div className="mb-6 border-b pb-4">
                      ...
                      <p className="text-sm text-gray-600">Fill in all required fields to register a new agent</p>
                    </div>
                          ...
                          <div className="mt-4 flex justify-center gap-3">
                            ...
                            {csvErrors.length === 0 && csvData.length > 0 && (
                              <ButtonModule variant="primary" onClick={handleBulkSubmit} disabled={isBulkLoading}>
                                {isBulkLoading ? "Processing..." : `Process ${csvData.length} Agents`}
                              </ButtonModule>
                            )}
                          </div>
                        </div>
