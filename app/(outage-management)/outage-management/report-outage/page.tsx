...
import { createOutage, CreateOutageRequest, fetchOutages } from "lib/redux/outageSlice"
...
const ReportOutage = () => {
  ...
  const { createLoading, createError, createSuccess } = useSelector((state: RootState) => state.outages)
  ...
  const [csvData, setCsvData] = useState<CSVOutage[]>([])
  ...
  const scopeOptions = [
    ...
    { value: "1", label: "Individual Customer" },
    { value: "2", label: "Area/Feeder" },
  ]
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.title.trim()) {
      errors.title = "Outage title is required"
  ...
  const handleSingleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    await submitSingleOutage()
  }

  const submitSingleOutage = async () => {
    if (!validateForm()) {
    ...
    try {
      const outageData: CreateOutageRequest = {
      ...
      const result = await dispatch(createOutage(outageData)).unwrap()

      notify("success", "Outage reported successfully", {
        description: `Outage "${formData.title}" has been reported and is being investigated`,
        duration: 5000,
      })
      ...
      const errorMessage = error || "An unexpected error occurred while reporting the outage"
      notify("error", "Failed to report outage", {
      ...
      try {
        ...
        if (errors.length === 0) {
          notify("success", "CSV file parsed successfully", {
            description: `Found ${parsedData.length} valid outage records`,
            duration: 4000,
          })
        } else {
  ...
  const validateCSVRow = (row: any, rowNumber: number): string[] => {
    ...
    if (!row.title?.trim()) {
      errors.push(`Row ${rowNumber}: Outage title is required`)
    } else if (row.title.length < 5) {
      errors.push(`Row ${rowNumber}: Title must be at least 5 characters long`)
    }

    if (!row.details?.trim()) {
      errors.push(`Row ${rowNumber}: Outage details are required`)
    ...
    if (!row.scope?.trim()) {
      ...
      errors.push(`Row ${rowNumber}: Scope must be 1 (Individual) or 2 (Area)`)
    }
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      // Process each outage in the CSV data
      const results = await Promise.allSettled(csvData.map((outage) => dispatch(createOutage(outage))))

      const successful = results.filter((result) => result.status === "fulfilled").length
      const failed = results.filter((result) => result.status === "rejected").length

      if (failed === 0) {
        notify("success", "Bulk outage report completed", {
          description: `Successfully reported ${successful} outages`,
          duration: 6000,
        })
      } else {
        notify("warning", "Bulk outage report completed with errors", {
          description: `Successfully reported ${successful} outages, ${failed} failed`,
          duration: 6000,
        })
      }
  ...
  const downloadTemplate = () => {
    const headers = ["title", "details", "distributionSubstationId", "feederId", "scope", "priority"]

    const exampleData = [
      {
        title: "Power Outage - Kaduna Central Area",
        details: "Complete power outage affecting residential areas in Kaduna Central due to transformer failure",
      ...
      {
        title: "Voltage Fluctuation - Barnawa District",
        details: "Voltage fluctuations causing equipment damage in Barnawa residential area",
    ...
    a.href = url
    a.download = "outage_report_template.csv"
            ...
            >
              ...
              >
                {activeTab === "single"
                  ...
                  : isBulkLoading
                  ? "Processing..."
                  : `Process ${csvData.length} Outages`}
              </ButtonModule>
            </motion.div>
          </div>
                ...
                {activeTab === "single" ? (
                  ...
                  >
                    ...
                    <div className="mb-6 border-b pb-4">
                      <h3 className="text-lg font-semibold text-gray-900">Outage Information</h3>
                      <p className="text-sm text-gray-600">Fill in all required fields to report a new outage</p>
                    </div>
                        ...
                        <div className="rounded-lg bg-[#f9f9f9] p-4">
                          ...
                          <div className="flex w-full justify-between gap-4 ">
                            <FormInputModule
                              ...
                              name="title"
                              type="text"
                              placeholder="Enter outage title (e.g., Power Outage - Kaduna Central)"
                            ...
                            <FormInputModule
                              ...
                              name="details"
                              type="textarea"
                              placeholder="Provide detailed description of the outage, affected areas, and any known causes"
                          ...
                          <div className="mt-4 flex justify-center gap-3">
                            ...
                            {csvErrors.length === 0 && csvData.length > 0 && (
                              <ButtonModule variant="primary" onClick={handleBulkSubmit} disabled={isBulkLoading}>
                                {isBulkLoading ? "Processing..." : `Process ${csvData.length} Outages`}
                              </ButtonModule>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="max-h-48 overflow-y-auto">
                          <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                              {csvData.slice(0, 5).map((outage, index) => (
                                <tr key={index}>
                                  <td className="px-4 py-2 text-sm text-gray-900">
                                    <div className="max-w-xs truncate">{outage.title}</div>
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
                                    {outage.distributionSubstationId}
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
                                    {outage.scope === 1 ? "Individual" : "Area"}
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-2 text-sm text-gray-900">
                                    {outage.priority === 1
                                      ? "Low"
